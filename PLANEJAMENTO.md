...
## 14.19 FASE 15 — GTM + GA4 (concluída)

- Auditoria completa confirmou a arquitetura de analytics já correta e pronta para receber um `NEXT_PUBLIC_GTM_ID` real: GTM como único ponto de integração (GA4 configurado dentro do container, nunca em paralelo — nenhum `gtag.js`/script direto do GA4 no código, nenhum Measurement ID hardcoded), `trackEvent()` como abstração central única, taxonomia de 5 eventos (`cta_click`/`whatsapp_click`/`diagnostic_start`/`diagnostic_step_complete`/`diagnostic_complete`) tipada de forma que PII é estruturalmente impossível de vazar (`EventParamsMap` só aceita `source`/`destination`/`step`, nunca um campo de formulário).
...
- **Bug real encontrado e corrigido**: `ConsentBanner.tsx` usava `inset-x-0 bottom-0` (ponta a ponta) no mobile, cobrindo por completo o botão flutuante do WhatsApp nessa faixa de tela — medido antes/depois via `getBoundingClientRect`. Corrigido unificando para o cartão ancorado à esquerda já usado a partir de `sm:` (`right-24` reserva a coluna do botão flutuante + 20px de respiro, matematicamente constante em qualquer largura de viewport já que os dois elementos são ancorados à mesma borda direita da tela), sem tocar em nenhum componente de WhatsApp. Revalidado em 320/360/390/768px — zero sobreposição, zero overflow, botões legíveis. Commit `049dfc1`.
- **Configuração externa concluída pelo usuário**: container GTM real (`GTM-MNNTCFS6`, "Configuração inicial GA4 + eventos Suite360" — 6 tags/5 gatilhos/8 variáveis) criado e publicado; propriedade GA4 real (`G-FMWXF5X63H`) conectada dentro do GTM; 5 gatilhos de Evento Personalizado + 5 tags GA4 Event cobrindo exatamente a taxonomia existente; variáveis de camada de dados mapeadas para `source`/`destination`/`step` (os únicos parâmetros que o código já enviava — nenhum parâmetro novo precisou ser criado). `NEXT_PUBLIC_GTM_ID` configurada na Vercel (Production). GTM Preview/Tag Assistant validou o domínio de produção, disparos reais observados, nenhum PII detectado.
- **Achado real desta rodada de fechamento**: verificado via inspeção do bundle JS servido em produção que o deployment ao vivo (o mesmo que já tem o GTM funcionando corretamente) **não inclui o commit `049dfc1`** — o JS ainda contém o padrão antigo `inset-x-0 bottom-0` do `ConsentBanner`, não a correção `right-24`. O redeploy que ativou `NEXT_PUBLIC_GTM_ID` aparentemente reaproveitou um build anterior a esse commit em vez de gerar um build novo a partir do `main` atual (ambos já sincronizados, nada pendente de push). Não foi feito nenhum redeploy automaticamente — o commit já está integralmente no repositório, só falta um novo deploy de produção a partir do `main` atual para essa correção específica entrar no ar; reportado ao usuário para decidir quando.
